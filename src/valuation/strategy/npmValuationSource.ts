...
export function parseNpmEvidence(raw: unknown, company: CompanyConfig, sourceUrl = "fixture://npm"): NpmEvidence {
  ...
  const latest = asRecord(record.latest_tape_d);
  ...
  const latestPrice = optionalNumber(latest.price ?? record.latest_npm_price);
  const tape = parseTape(record.tape_d_prices, latestTapeDate, latestValuation, latestPrice);
