...
class HoldingRecord:
    # None means flat: the bot holds no YES leg on the grouped event.
    held_location: str | None
    source: str  # "config_default" | "entry" | "rotation" | "exit" | "rotation_incomplete"
    updated_at: str
...
class HoldingsStore:
    """Persistent record of WHICH outcome the bot currently holds YES on.

    `event.held_location` in the YAML is only the initial/default holding.
    Once automated entry or rotation runs, the live holding diverges from the
    config, and every later decision (protection sells, time decay, price
    ...
    still names the sold leg cannot resurrect it as held.
    """
